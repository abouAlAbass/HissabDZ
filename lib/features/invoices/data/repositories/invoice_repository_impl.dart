...
class InvoiceRepositoryImpl implements InvoiceRepository {
  ...
  @override
  Future<List<entity.Invoice>> getInvoices({InvoiceStatus? status, String? query}) async {
    final statement = _database.select(_database.invoices).join([
      leftOuterJoin(_database.clients, _database.clients.id.equalsExp(_database.invoices.clientId)),
    ]);
    ...
    if (query != null && query.isNotEmpty) {
      statement.where(_database.invoices.invoiceNumber.like('%$query%') | _database.clients.name.like('%$query%'));
    }
    ...
    return Future.wait(rows.map((row) async {
      ...
      return _mapToEntity(invoiceRow, clientRow, items);
    }));
  }
  ...
  Stream<List<entity.Invoice>> watchInvoices({InvoiceStatus? status}) {
    ...
    return query.watch().asyncMap((rows) => Future.wait(rows.map((row) async {
      final invoiceRow = row.readTable(_database.invoices);
      final clientRow = row.readTableOrNull(_database.clients);
      final items = await _getInvoiceItems(invoiceRow.id);
      return _mapToEntity(invoiceRow, clientRow, items);
  ...
  Future<int> createInvoice(entity.Invoice invoice) async {
    return _database.transaction(() async {
      final invoiceId = await _database.into(_database.invoices).insert(
  ...
  Future<void> updateInvoice(entity.Invoice invoice) async {
    return _database.transaction(() async {
      await _database.update(_database.invoices).replace(
      ...
      await (_database.delete(_database.invoiceItems)..where((t) => t.invoiceId.equals(invoice.id!))).go();
  ...
  Future<void> deleteInvoice(int id) {
    return (_database.delete(_database.invoices)..where((t) => t.id.equals(id))).go();
  }

  @override
  Future<String> generateNextInvoiceNumber() async {
    final query = _database.select(_database.invoices)..orderBy([(t) => OrderingTerm.desc(t.id)])..limit(1);
    final lastInvoice = await query.getSingleOrNull();
    
    if (lastInvoice == null) return 'INV-0001';
    
    final lastNumber = int.tryParse(lastInvoice.invoiceNumber.split('-').last) ?? 0;
    return 'INV-${(lastNumber + 1).toString().padLeft(4, '0')}';
  }

  Future<List<entity.InvoiceItem>> _getInvoiceItems(int invoiceId) async {
    final rows = await (_database.select(_database.invoiceItems)..where((t) => t.invoiceId.equals(invoiceId))).get();
    return rows.map((row) => entity.InvoiceItem(
      id: row.id,
  ...
  entity.Invoice _mapToEntity(db.InvoiceData invoiceRow, db.ClientData? clientRow, List<entity.InvoiceItem> items) {
    return entity.Invoice(
      ...
      items: items,
      client: clientRow != null ? entity.Client(
        id: clientRow.id,
        name: clientRow.name,
